...
- 0run.ps1 会自动强杀现有 xOpenTerm 进程、清除日志后再启动
...
- `.run/` — 0run.ps1 运行时工作目录（.run\config、.run\log）
- `.temp/` — 编译输出目录
- `.dist/` — 发布目录
- `doc/`、`aidoc/` — 文档目录
...
### 日志系统
- 日志位置：`<当前工作目录>/log/`（0run.ps1 下即 .run/log/）
...
- 运行方式：`dotnet test` 或 `./0run.ps1 --test-ssh-status`
...
- 不允许在代码中硬编码密码、API 密钥等敏感信息### Agent 结束后自动执行 0run.ps1
Agent 执行结束后会自动运行 `./0run.ps1` 进行构建和测试
