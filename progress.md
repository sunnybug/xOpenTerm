...
rdp节点图标改为win图标
新增一种特殊的父节点：腾讯云。1.节点数的普通右键菜单增加：腾讯云组，新增时需要填入腾讯云的密钥 2.新增腾讯云组节点时，将腾讯云上所有服务器导入到到该父节点中，节点树结构：一级是机房（香港/广州），二级是项目，三级是服务器节点（linux用SSH,windows用RDP），所有服务器节点要保存腾讯云上的资源id来标记服务器唯一id 3.服务器节点的登录凭证默认是同父节点 4. 腾讯云组类型的父节点的右键菜单相对于普通父节点多一个：同步（执行腾讯云节点同步功能）。同步时需要显示同步进度,可以取消。同步时若本地资源id在腾讯云上已不存在，则删除本地节点。5.新增的腾讯云组节点下所有登录凭证默认是同父节点。
认证选择同父节点/登录凭证时，用户名控件不应显示，控件中的值也不应该被使用
连接tab上右键菜单显示：重连/断开（仅仅是断开而不清除tab内容）/关闭
从腾讯云同步的节点要显示项目名，目前显示的是项目id
右键菜单中的连接全部需要提示用户
节点树右键菜单新增：导出-yaml。导出为yaml文件，包含节点数结构和登录信息。若有登录凭证，则将登录凭证一并导出。导入-yaml，用于导入yaml文件，若导入了登录凭证相同的，则忽略，不同的（用户名密码）则创导入。
关闭连接tab后，若无连接tab，则激活服务器tab（若已为服务器tab则无操作）。若尚有连接tab，则按右>左顺序切换到下一个连接tab。
从节点树打开连接时，不要自动切换到远程文件tab
切换连接tab时，若已激活远程文件tab，则试试切换远程文件tab为当前连接tab的远程文件tab
远程文件tab若打开则一直缓存直到对应的连接tab关闭
连接tab上显示节点图标
关闭连接tab时要提示用户
退出程序时若有连接tab开着，要提示用户
连接tab不要总是铺满整行
对于腾讯云组下的所有父节点（无论嵌套多深），都不允许导入或新建腾讯云组节点。即云组类型父节点下不能嵌套云组类型父节点。
同步时要显示详细的服务器变动列表
阿里云/腾讯云同步多线程支持，轻量云和cvm多地域并行拉取
阿里云/腾讯云父节点的设置支持设置accesskeyid和accesskeysecret
...
内存/cpu/上传/下载都需要有各自的面积图
参考阿里云，新增金山云导入和同步功能和相关界面
导入Mobaxterm时，提供MobaXterm.ini和密码文件（由mobaxterm导出）若提供密码文件，则若读取到密码类型的节点，且用户名为[配置名]格式，则将此用户名作为key从密码文件中读取用户名和密码。密码文件格式：配置名(真正的用户名) = 真正的密码
节点树中，右键菜单触发时，选择的节点变为鼠标处的节点
...
菜单中有些快捷键重复
腾讯云的详情示例：https://console.cloud.tencent.com/cvm/instance/detail?searchParams=rid%3D5&rid=5&id=ins-xxx
阿里云的详情示例：https://ecs.console.aliyun.com/server/i-xxx/detail?regionId=cn-hongkong#/
新建云组时，父节点的设置中，登录凭证默认设置为：同父节点。
云组节点右键菜单-同步，改为同步子菜单，有2个菜单：同步和重建。重建是将云组节点下所有节点删除，再同步，需先提示用户。
父节点显示服务器个数
ssh状态栏的右键菜单和上面的tab右键菜单分开。ssh状态栏的流量上的右键菜单为：查看进程流量，会在剪贴板存入命令行：显示进程流量（按流量大小排序）。注意要根据不同的操作系统和工具安装情况来提供不同命令行，若所需工具均未安装则提示先安装工具并提供安装命令行
右边的连接tab右键菜单增加：关闭所有连接。删除关闭菜单
ssh状态栏中的磁盘空间超过90%时，显示红色。磁盘空间右键菜单：查找占用空间最大的文件/目录（注意要通过系统信息收集模块，根据操作系统来提供不同命令行，若所需工具均未安装则提示先安装工具并提供安装命令行）
节点右键菜单新增子菜单：维护，里面有：磁盘占用。磁盘占用：遍历父节点下所有子节点（若为子节点则检查当前节点），若有磁盘占用超过xx%的节点，则显示该节点名称和磁盘占用率，并收集磁盘占用空间最大的文件/目录，并显示在界面中。点击文件/目录后，会在ssh连接tab中输入命令行打开该文件/目录。xx%默认85,待用户输入。

大文件与多文件传输
在当前ssh标签页的终端输出中搜索关键词（高亮/跳转）
同一 SSH 连接下多窗格分屏（左右/上下），共享同一会话
一键隐藏或折叠左侧服务器树
端口扫描

bug:
进程打开某个对话框后，转到其他进程，再转回来，有概率会看不到这个打开的对话框，这样会导致无法操作该进程的界面
子节点设置中的下拉框风格和父节点设置中的下拉框风格要一致
