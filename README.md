...
- **维护 - 端口扫描**：支持所有主机节点（SSH 和 RDP）右键菜单 → 维护 → 端口扫描；支持自定义端口范围（格式：`22,80,443` 或 `1-1024` 或混合输入）、**可配置端口预设**（Top 20、Web服务、数据库、SSH常见端口、所有端口等）、端口范围历史记录（最多保存20条，可快速选择/清除）；**统一从本机发起 TCP 连接扫描**（无论节点类型是 SSH 还是 RDP，都从本机直接连接目标节点的端口进行检测），确保扫描结果反映从本机到目标节点的实际网络可达性；支持端口级并发扫描（单个节点可同时扫描多个端口，可配置1-1000个端口并发，默认50）、超时设置（1-10秒）；扫描结果**实时显示**，节点默认折叠，扫描到开放端口时节点标题显示**红色**，未发现开放端口时节点标题显示**绿色**，支持展开查看端口详情；扫描中按钮变为"停止扫描"，可随时中断；配置自动保存到 `config/settings.yaml`（默认并发数、超时、**上次选择的端口预设**、端口历史记录）；**端口预设管理**：点击"管理预设"按钮可打开预设管理窗口，支持添加、编辑、删除端口预设方案，修改后立即刷新主窗口下拉框
...
- **顶栏菜单**：设置 → 登录凭证、隧道管理、端口扫描预设、恢复配置；帮助 → 关于、检查更新
...
- `0run.ps1` — 构建并运行应用（支持 --release、--test-rdp、--test-ssh-status）
...
# 仅运行单元测试
.\0run.ps1 --test-ssh-status  # SSH 状态获取测试（root@192.168.1.192，SSH Agent，连接超时 3s，无 UI，测试结束自动退出）
.\0run.ps1 --test-scan-port   # 端口扫描功能测试（打开 UI 并自动执行端口扫描，扫描完成后延迟 3 秒自动退出）
...
.run/ 运行时工作路径（0run.ps1 与 dotnet test 单元测试均使用），其下 log/、config/
0run.ps1
```
...
- 将工程的工作目录设置为 .run（0run.ps1）或项目根（或 launch 指定）
...
### v1.7.0 (2026-02-20)
- **重构**：统一端口扫描架构，SSH 和 RDP 节点都从本机发起 TCP 连接扫描
  - **问题**：之前 SSH 节点通过 SSH 连接执行远程 nc 命令扫描，RDP 节点从本机扫描，架构不一致
  - **改进**：统一从本机发起 TCP 连接检测，确保扫描结果反映从本机到目标节点的实际网络可达性
  - **好处**：
    - 架构统一，SSH 和 RDP 节点使用相同的扫描逻辑
    - 扫描结果更准确，反映真实的网络可达性
    - 不依赖远程机器上的工具（nc、bash 等）
    - 可以检测防火墙规则是否允许本机访问
  - **修复**：`LocalPortScanner.ScanPortAsync` 的连接判断逻辑，确保能正确区分连接成功、连接被拒绝、超时等状态
  - **测试验证**：
    - 本地扫描：127.0.0.1:3389 (RDP) 正确识别为开放
    - 本地扫描：127.0.0.1:22/80/443/3306 正确识别为关闭
    - 公网扫描：149.129.223.30:80 正确识别为开放 (HTTP)
    - 所有单元测试通过（40 个测试）
