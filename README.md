...
- `start.ps1` — 构建并运行应用（支持 --release、--test-rdp、--test-ssh-status）
...
.run/ 运行时工作路径（start.ps1 与 dotnet test 单元测试均使用），其下 log/、config/
start.ps1
```
...
- 将工程的工作目录设置为 .run（start.ps1）或项目根（或 launch 指定）
