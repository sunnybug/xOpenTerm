...
- **维护 - 磁盘占用**：节点（分组或 SSH 主机）右键菜单 → 维护 → 磁盘占用；输入占用率阈值（默认 85%），对当前节点或分组下所有 SSH 子节点执行检查；超过阈值的节点会列出并采集根目录下占用最大的文件/目录；结果中点击某条路径可在对应 SSH 标签页中粘贴「cd 路径」命令（已复制到剪贴板，按 Ctrl+V 执行）
