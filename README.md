...
- **维护 - 磁盘占用**：节点（分组、SSH 或云类型 RDP）右键菜单 → 维护 → 磁盘占用；对 **SSH** 节点输入占用率阈值（默认 85%），对当前节点或分组下所有 SSH 子节点执行远程 df/du 检查，超过阈值的列出并采集根目录下占用最大的文件/目录；对 **云类型 RDP** 节点（腾讯云/阿里云/金山云同步的 RDP）通过云厂商 API 查询实例磁盘容量（系统盘、数据盘 GB），在结果中展示
