...
public partial class MasterPasswordWindow : Window
{
    ...
    /// <param name="isSetMode">true=设置主密码（需确认），false=输入主密码（仅校验并设置会话密钥）</param>
    /// <param name="salt">输入模式下的盐（Base64 解码后的字节）</param>
    /// <param name="verifier">输入模式下的验证码（Base64 解码后的字节）</param>
    public MasterPasswordWindow(bool isSetMode, byte[]? salt, byte[]? verifier)
    {
        InitializeComponent();
    ...
    private void OkBtn_Click(object sender, RoutedEventArgs e)
    {
        ...
        // 输入模式：校验主密码并设置会话密钥，并保存到本地文件以便下次启动无需输入
